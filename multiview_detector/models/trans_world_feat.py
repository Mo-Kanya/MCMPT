...
class DeformTransWorldFeat(nn.Module):
    def __init__(self, num_cam, Rworld_shape, base_dim, hidden_dim=128, dropout=0.1, nhead=8, dim_feedforward=512,
        ...
        self.pos_embedding = create_pos_embedding( (82,106), hidden_dim // 2) # TODO: np.array(Rworld_shape) // stride
    ...
    def forward(self, x, visualize=False):
        ...
        # print(B, N, C, H, W)
        # print(self.pos_embedding.shape, self.lvl_embedding.shape)
        # print(self.pos_embedding.to(x.device).flatten(2).transpose(1, 2).unsqueeze(1).shape)
