...
# TODO: what is z here? why 0s?
def create_reference_map(dataset, n_points=4, downsample=2, visualize=False, proj_mat=None):
    if proj_mat is None:
        raise NotImplementedError
    H, W = dataset.Rworld_shape  # H,W; N_row,N_col
    H, W = 82,106  # H // downsample, W // downsample
...
class MVDeTr(nn.Module):
    def __init__(self, dataset, arch='resnet18', z=0, world_feat_arch='conv',
        ...
        map_zoom_mat = np.diag(np.append(np.ones([2]) / dataset.world_reduce, [1]))
        self.proj_mats = torch.stack([torch.from_numpy(map_zoom_mat @ imgcoord2worldgrid_matrices[cam])
        ...
        if arch == 'vgg11':
            ...
            self.base = nn.Sequential(*list(resnet18(pretrained=True,
                                                     replace_stride_with_dilation=[True, True, True]).children())[:-2])
            base_dim = 512
        else:
        ...
        if world_feat_arch == 'conv':
            ...
            n_points = 4
            reference_points = create_reference_map(dataset, n_points, proj_mat=self.proj_mats.numpy()).repeat([dataset.num_cam, 1, 1, 1])
    ...
    def forward(self, imgs, M, visualize=False):
        ...
        if visualize:
            ...
            proj_imgs = kornia.geometry.transform.imgwarp.warp_perspective(T.Resize(self.Rimg_shape)(imgs), proj_mats.to(imgs.device),
        ...
        world_feat = kornia.geometry.transform.imgwarp.warp_perspective(imgs_feat, proj_mats.to(imgs.device),
