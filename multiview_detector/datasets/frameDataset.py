...
class frameDataset(VisionDataset):
    def __init__(self, base, train=True, reID=False, world_reduce=4, img_reduce=12,
        ...
        self.imgs_region = kornia.geometry.transform.imgwarp.warp_perspective(world_masks, self.img_from_world, self.img_shape, 'nearest',
                                                   align_corners=False)
...
class MMPframeDataset(VisionDataset):
    ...
    def __init__(self, config_file, train=True, sample_freq=1, dropout=0.0,
                 world_kernel_size=10, img_kernel_size=10, world_reduce=2, img_reduce=4,
                 top_k=20, force_download=False, augmentation=False):
        ...
        # map_sigma, map_kernel_size = 20 / world_reduce, 20
        # img_sigma, img_kernel_size = 10 / img_reduce, 10
        # map_sigma, map_kernel_size = 16 / world_reduce, 16
        ...
        # self.Rworld_shape = self.grid_size  # assuming no reduction
    ...
    def __getitem__(self, index):
        ...
        for cam in range(1, self.num_cam+1):
            ...
            # TODO: implement and test with augmentation; maybe also drop cam
            # if self.augmentation:
            #     img, img_bboxs, img_pids, M = random_affine(img, img_bboxs, img_pids)
            # else:
            M = np.eye(3)
            ...
            for pid in label:
                ...
                fr.append(y_foot)
                c.append(x)
                ws.append(max(min( int(bbox[2] - bbox[0]), self.img_shape[1] - 1), 0))
                hs.append(max(min( int(bbox[3] - bbox[1]), self.img_shape[0] - 1), 0))
            # print(hr)
            # print(c)
            img_gt = get_gt(self.Rimg_shape, c, fr, ws, hs, v_s=pids,
                            reduce=self.img_reduce, top_k=self.top_k, kernel_size=self.img_kernel_size)
        ...
        with open(os.path.join(self.topdown_label_path, img_dir, 'topdown_' + str(new_index).zfill(5) + '.csv'), 'r') as csv_file:
            ...
            for row in csv_reader:
                ...
                i_s.append( min(int(float(row[1])), self.worldgrid_shape[0])-1 )
                j_s.append( min(int(float(row[2])), self.worldgrid_shape[1])-1 )
            # map_gt = coo_matrix((v, (i_s, j_s)), shape=self.Rworld_shape).toarray()
            map_gt = get_gt(self.Rworld_shape, j_s, i_s, v_s=pids,
                            reduce=self.world_reduce, top_k=self.top_k, kernel_size=self.world_kernel_size)
            # if self.target_transform is not None:
...
def test(test_projection=False):
    ...
    import matplotlib.pyplot as plt
    fig = plt.figure()
    subplt1 = fig.add_subplot(211, title="cam")
    subplt2 = fig.add_subplot(212, title="top")

    mmp = MMPframeDataset("/home/kanya/MVDet/multiview_detector/datasets/configs/retail.yaml", train=True)
    imgs, map_gt, imgs_gt, affine_mats, new_index = mmp[15]
    ...
    # print(map_gt["heatmap"].shape)

    subplt1.imshow(map_gt['heatmap'].detach().numpy().transpose([1, 2, 0]))
    ...
    dataset = frameDataset(MultiviewX(os.path.expanduser('~/Data/MultiviewX')), train=True, augmentation=True)
