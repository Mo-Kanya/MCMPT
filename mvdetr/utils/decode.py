...
from mvdetr.utils.tensor_utils import _gather_feat, _transpose_and_gather_feat
...
def mvdet_decode(scoremap, offset=None, reduce=4):
    ...
    xy = torch.nonzero(torch.ones_like(scoremap[:, 0])).view([B, H * W, 3])[:, :, [2, 1]].float() # B x H*W x 2 (indices)
    if offset is not None:
