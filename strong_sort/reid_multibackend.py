...
class ReIDDetectMultiBackend(nn.Module):
    ...
    def preprocess(self, im_crops):
        ...
        for im_crop in im_crops:
            if im_crop.shape[1] == 0:
                continue
            im.append(self.norm(_resize(im_crop, self.size)).unsqueeze(0))
        im = torch.cat(im, dim=0)
        # im = torch.cat([self.norm(_resize(im, self.size)).unsqueeze(0) for im in im_crops if im.shape[0] != 0], dim=0).float()
